...
public interface CustomerBedHistoryRepository extends JpaRepository<CustomersBedHistory, Long> {
    ...
    @Query(value = """
            SELECT * FROM customers_bed_history WHERE customer_id=:customerId and DATE(start_date) <= DATE(:startDate) and (end_date IS NULL OR DATE(end_date) >= DATE(:endDate))
    ...
    @Query("""
                ...
                WHERE h.customerId = :customerId
                  ...
                  AND (h.endDate IS NULL OR h.endDate <= :endDate)
