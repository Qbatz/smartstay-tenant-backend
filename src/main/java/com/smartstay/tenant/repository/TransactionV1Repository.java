...
public interface TransactionV1Repository extends JpaRepository<TransactionV1, String> {
    ...
    @Query(value = """
            ...
            INNER JOIN (
                ...
                ON latest.invoice_id = t.invoice_id
                AND latest.latest_paid_at = t.paid_at
            WHERE t.invoice_id IN (:invoiceIds)
            """, nativeQuery = true)
    List<TransactionV1> findLatestTransactionsByInvoiceIds(@Param("invoiceIds") Set<String> invoiceIds);
}
